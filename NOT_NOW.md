...
- **Downloadable workplace artifact templates** (`templates/manual-testing/`: Test Case Template, RTM Template, Test Data Template, all `.xlsx`) — distinct from the contributor template library above; these are ready-to-use artifacts for *learners* to download and use on the job, not templates for writing TestAtlas content. Modules 10 (Writing Clear Test Cases), 12 (RTM), and 13 (Test Data Design) are the natural anchors — each already notes "Downloadable Resources (Coming Soon)" pointing here. Deferred because building real, polished `.xlsx` templates is its own deliverable requiring actual spreadsheet design, not something to produce ad hoc mid-module-writing. Revisit as a dedicated, separately-scoped milestone once enough of Manual Testing's artifact-teaching modules exist to build the full set at once, rather than one at a time.
