...
| **Application Modules** — `difficulty: "intermediate"`, Quick Revision retained, Mini Challenge and the three narrative callouts omitted, no Section Review/Solutions | `CURRICULUM_EVOLUTION.md` Entry 2 (corrected 2026-08-04) | Manual Testing Modules 20–21, 23 (Module 22 is a known, deliberately-unretrofitted exception — see Entry 2's Correction); API Testing Modules 18–21; Automation Testing Modules 15–18; Database Testing Modules 13–16 |
| **AtlasBank** as the default recurring domain, with explicit exception for a genuinely better-fitting domain | `STYLE_GUIDE.md` | Manual Testing from Section 3 onward; all of API Testing (extended 2026-08-04), Automation Testing, and Database Testing — Database Testing additionally introduces **AtlasShop**, a named contrasting domain, for its second Application Module (approved 2026-08-05, the first path to name a second recurring domain rather than leaving it unnamed or staying within AtlasBank exclusively) |
...
**Next path**: with four certified Reference Curricula now shipped (Manual Testing, API Testing, Automation Testing, Database Testing — 78 modules total), the user's own stated direction is to pause new-curriculum work here: cut a v1.1.0 milestone covering this batch, then decide the next learning path based on actual user/community feedback rather than starting a fifth path immediately. Whichever path comes next should still be architected as a proposal against this certification's defaults, following Automation Testing's and Database Testing's own example of a written architecture proposal reviewed and approved before any module content is written — but the trigger for starting it is feedback, not this document alone.
